...
class Compliance:
    def __init__(self, rg=orm.Refget()) -> None:
        self.rg = rg
    ...
    def populate(self, silent=False):
        ...
        with rg.session.begin():
            ...
            self._load("6681ac2f62509cfc220d78751b8dc524", "BK006935.2", "insdc", "dna")
            if not silent:
                print("Load S.cer chr VI")
            self._load("b7ebc601f9a7df2e1ec5863deeae88a3", "BK006940.2", "insdc", "dna")
            if not silent:
                print("Load ACGT")
            raw_seq, mol = self.create("ACGT", "basic", "none", "dna", False)
            rg.session.add_all((raw_seq, mol))
            rg.session.commit()

    def create(self, seq, id, authority, seq_type="dna", circular=False):
        rg = self.rg
    ...
    def _load(self, checksum, id, authority, seq_type, circular=False):
        rg = self.rg
        ena_refget = "https://www.ebi.ac.uk/ena/cram/sequence"
        r = requests.get(f"{ena_refget}/{checksum}")
        seq = r.text
        raw_seq, mol = self.create(seq, id, authority, seq_type, circular=circular)
        rg.session.add_all([raw_seq, mol])
